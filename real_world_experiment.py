...
if __name__=="__main__":
    ...
    # we load our dataset
    data = pd.read_csv("./volatility_data/AAPL_20_year_daily_vol.csv")
    xs = normalize(data["returns"].to_numpy())
    ys = normalize(data["volatility"].to_numpy())

    xs = xs[:5000].reshape((100, 50, 1))
    ys = ys[:5000].reshape((100, 50, 1))
